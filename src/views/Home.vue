    ...
    <div class="container">
        <div class="min-h-[calc(100vh-160px)]">
            <div class="flex  gap-x-5  py-4">
                    ...
                    <div v-else class="flex flex-col">
                       <div class="bg-white p-4 rounded-xl mb-5">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center justify-center text-blue-600 font-semibold">
                                    <span class="font-bold text-[100px]">{{ temperature }}</span> <sup class="text-[50px]">&deg; C</sup>
                                </div>
                        ...
                        <div class="flex items-center justify-between mb-10">
                            <template v-for="(hour, index) in hours.slice(0, 8)" :key="index"> 
                              
                                <div class="hour-card bg-white border border-green-400 w-[100px] rounded-lg p-2 shadow-md">
                                    
                                    <p class="text-center text-[14px]">
                                        {{ new Date(hour.dt * 1000).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) }}
                                    </p>
                                    
                                    <img :src="'https://openweathermap.org/img/wn/' + hour.weather[0].icon + '@2x.png'" alt="weather" class="w-[50px] object-cover mx-auto">
                                    
                                    <p class="text-center text-[15px] mb-2">{{ hour.weather[0].main }}</p>

                                    <p class="flex items-center justify-center font-semibold">
                                        <span class="text-[10px]">{{ hour.temp }}</span>

                                        <sup class="text-[8px]">&deg; C</sup>
                                    </p>
                                </div>
                            </template>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-4">
                            <template v-for="(item, index) in dayNames" :key="index"> 
                                <div class=" bg-white border border-green-400 py-4 border border-gray-200 rounded-xl">
                                    <p class="text-center">
                                        {{
                ...
                <div class="w-[25%] bg-white py-4 px-2 rounded-lg self-start">
                    <h2 class="mb-5 text-center font-bold text-xl">Regions</h2>

                    <ul class="flex flex-col gap-y-2 text-md cursor-pointer w-full">
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('tashkent')">
                        ...
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('karakalpakstan')">
                        ...
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('jizzakh')">
                        ...
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('sirdaryo')">
                        ...
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('khujand')">
                        ...
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('fergana')">
                            <label for="fr" class="flex items-center justify-between w-full cursor-pointer">
                                <span>Fergana</span>
                                <input id="fr" type="radio" name="r">
                            </label>
                        </li>
                        <li class="hover:bg-green-300 p-2 rounded-md" @click="getName('urgench')">
...
<script setup>
    ...
    const getDatafor7days = async (lat, lon) => {
        loader.value = true
        let url = `https://api.openweathermap.org/data/2.8/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=minutely,alerts&appid=${apiKey}`
        try {
            const res = await axios(url);
            const data = res.data;
